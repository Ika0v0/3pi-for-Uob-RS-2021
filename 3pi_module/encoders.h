...
// Volatile 指每次使用该变量时确保重新读值使值实时准确
...
ISR( PCINT0_vect ) {
  ...
  // Non-standard pin, so we access the register directly.
  ...
  // Refer to the labsheet.     // the code doesn't change because encoders are symmetric
